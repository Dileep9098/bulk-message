...
export default function Calling() {
    ...
    const handleCallUsers = async () => {
        const numbers = allUser
            ...
            .map((user) => user.whatAppNo); // Or mobile

        if (numbers.length === 0) {
            showErrorMsg("Select at least one user to call.");
            return;
        }
        if (!message.trim()) {
        ...
        try {
            const response = await axiosInstance.post(Config.END_POINT_LIST["MAKE_CALL"], { numbers,message});

            if (response.data.success) {
                showSuccessMsg("Call initiated successfully!");
            } else {
                showErrorMsg(response.data.message || "Call failed");
            }
        } catch (error) {
            showErrorMsg(error.response?.data?.message || "Call request failed!");
        }
    };
